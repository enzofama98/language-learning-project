 ...
 const loadExercisesAndProgress = async (userId: string) => {
    try {
      ...
      const { data: exercises, error: exercisesError } = await supabase
        ...
        .select("*")
        .eq("language_code", courseCode.toUpperCase())
        .eq("lezione", lessonNumber) // FILTRA PER LEZIONE
        .eq("active", true)
...
const goToNextExercise = () => {
  ...
  if (currentExerciseIndex < currentLesson.exercises.length - 1) {
    ...
    // Siamo all'ultimo esercizio - completa la lezione
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="text-center">
        ...
        <button
          onClick={() => router.push(`/course/${courseCode}/lessons`)} // <-- NON "/" ma "/course/.../lessons"
          className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-md"
        >
          Torna alle Lezioni
        </button>
    ...
    <div className="max-w-4xl mx-auto px-4 py-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button
            onClick={() => router.push(`/course/${courseCode}/lessons`)}
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              Lezione {lessonNumber} - Esercizio {currentExerciseIndex + 1} di {currentLesson?.exercises.length || 0}
            </p>
          </div>
        </div>
        <button
